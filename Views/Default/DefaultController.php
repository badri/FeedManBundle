	 ...
	 public function addAction(){
		...
		$form = $this->createFormBuilder($feed)
			...
			->add('schedule_day', 'choice', array(
				'label' => 'Schedule day of week to send email',
				'multiple' => false,
				'choices' => $days,
			...
			->add('save','submit', array('attr' => array('class' => 'btn button'),'label' => "Add Feed"))->getForm();
	 ...
	 public function editAction(){
		...
		if(!empty($fId)){
			...
			$form = $this->createFormBuilder($feed)
				...
				->add('save','submit', array('attr' => array('class' => 'btn button'),'label' => "Edit Feed"))->getForm();
			 $form->handleRequest($this->request);
			 if ($form->isSubmitted()){
				 if ($form->isValid()) {
			...
			return $this->delegateView(array(
				...
				'passthroughVars' => array(
					'activeLink'     => '#feedman_index',
					'mauticContent'  => 'mydemo',
					'route'          => $this->generateUrl('feedman_editfeed',array('feedid'=>$feed->getId()))
				),
