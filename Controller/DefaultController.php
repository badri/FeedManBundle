...
class DefaultController extends CommonFormController 
{
    ...
    {	
		...
		if(empty($delIds) && empty($delId) ){
			...
			$this->addFlash(
						'feedman.feed.notice.deletesuccess'
			);
			return $this->delegateView(array(
				  'viewParameters'  => $viewParameters,
				'contentTemplate' => 'FeedManBundle:Default:list.html.php',
	 ...
	 public function addAction(){
		...
		$feedStatus = $this->factory->getParameter('status');
		...
		$form = $this->createFormBuilder($feed)
			...
			->add('leadlist_id', 'choice', array(
				...
				'label' => 'feedman.feed.status',
				'multiple' => false,
				'choices' => $feedStatus,
				'attr' => array('style' => 'width:300px', 'customattr' => 'customdata'),
			))->add('save','submit', array('attr' => array('class' => 'btn btn-default'),'label' => 'feedman.feed.addfeed'))->getForm();
		 ...
		 if ($form->isSubmitted()){
			 if ($form->isValid()) {
				...
				$prevFeeds = $this->getDoctrine()->getRepository('FeedManBundle:FeedData')->findBy( array('feed_id' => $feed->getId()));
				...
				foreach ($this->simplePieObj->get_items() as $item) {
					...
					if(!in_array($item->get_id(true), $prevFeedsHash)){
						...
						$feedData->setHash($item->get_id(true));
			    ...
			    $viewParameters = array('feedid' => $feed->getId());
			    $postActionVars = array(
					'returnUrl' => $this->generateUrl('feedman_selectfeed',array('feedid'=>$feed->getId())),
					'viewParameters'  => $viewParameters
				);
				/*
	 ...
	 public function editAction(){
		...
		if(!empty($fId)){
			...
			->add('leadlist_id', 'choice', array(
				...
				'multiple' => false,
				'choices' => $feedStatus,
			))->add('send_feed_type','choice',array(
				'choices'=>(array)$feed_send_type,
				'multiple' => false,
				'expanded' =>true,
				'label'=>'Select which feeds should be sent:'
			))->add('save','submit', array('attr' => array('class' => 'btn btn-default'),'label' => "feedman.feed.editfeed"))->getForm();
			 $form->handleRequest($this->request);
			 if($form->isSubmitted()){
				 if ($form->isValid()) {
					...
					$formData = $this->request->get('form');
					$selectFeedType = $formData['send_feed_type'];
					...
					switch($selectFeedType){
						...
						case 'manually_select':
							$feed->setSendFeedType('manually_select');
					...
					$this->addFlash('feedman.feed.notice.updatesuccess');
					return $this->delegateView(array(
					'viewParameters'  => $viewParameters,
					'contentTemplate' => 'FeedManBundle:Default:list.html.php',
					'passthroughVars' => array(
	 ...
	 public function selectfeedAction($feedId=''){
		...
		$form = $this->createFormBuilder($feed)->add('send_feed_type','choice',array(
				...
				'expanded' =>true,
				'label'=>'Select which feeds should be sent:'
			))->add('save','submit', array('attr' => array('class' => 'btn button'),'label' => "feedman.feed.submit"))->getForm();	
		 if ($this->request->isMethod('POST')) {
			...
			switch($selectFeedType){
				...
				break;
				case 'manually_select':
					$currentFeed->setSendFeedType('manually_select');
					if(count($selectedFeeds) > 0){
						$selectedFeedsString = implode(",",$selectedFeeds);						
					}
			...
			$this->addFlash(
						'feedman.feed.notice.addsuccess'
				);
